...
def FunctionSoftsplat(tenInput, tenFlow, tenMetric, strType):
    ...
    if strType == 'average':
        ...
        temshap = [i for i in tenInput.shape]
        temshap[1] = 1
        #print(temshap,tenInput.shape)
        tenInput = (tenInput +1)/2
        if(tenMetric == None):
            tenInput = torch.cat([ tenInput * 1, torch.ones(temshap).to(tenInput.get_device()) ], 1) # 1 = tenMetric.exp() wird zweimal ersetzt!
        else:
